...
var allureReporter__default = /*#__PURE__*/_interopDefaultLegacy(allureReporter);
...
const globPromise = util__default["default"].promisify(glob__default["default"]);
...
const helpers = {
  ...
  getVideoPath(testname) {
      return path__default["default"].resolve(config.outputDir, testname + this.getVideoFileExtension());
  },
      ...
      .then(frames => {
        ...
        if (frames.length) {
          ...
          const insertMissing = (sourceFrame, targetFrame) => {
            ...
            insertionPromises.push(fs__default["default"].copy(src, dest, options));
          };
  ...
  waitForVideosToExist(videos, abortTime) {
    ...
    do {
      this.sleep(100);
      allExist = videos
        .map(v => fs__default["default"].existsSync(v))
        .reduce((acc, cur) => acc && cur, true);
      if (allExist) {
        allGenerated = videos
          .map(v => fs__default["default"].statSync(v).size)
  ...
  waitForVideosToBeWritten(videos, abortTime) {
    ...
    do {
      this.sleep(100);
      let currentSizes = videos.map(filename => ({filename, size: fs__default["default"].statSync(filename).size}));
...
var defaultFramework = {
  ...
  onTestStart (test) {
    ...
    this.frameNr = 0;
    this.recordingPath = path__default["default"].resolve(config.outputDir, config.rawPath, this.testname);
    mkdirp__default["default"].sync(this.recordingPath);
  },
  ...
  onTestSkip () {
    if(this.recordingPath !== undefined) {
      fs__default["default"].removeSync(this.recordingPath);
    }
  },
...
var cucumberFramework = {
  ...
  onSuiteStart (suite) {
    ...
    if (suite.type === 'scenario') {
      ...
      this.frameNr = 0;
      this.recordingPath = path__default["default"].resolve(config.outputDir, config.rawPath, this.testname);
      if (!fs__default["default"].existsSync(this.recordingPath)) {
        mkdirp__default["default"].sync(this.recordingPath);
      }
    }
  },
  ...
  onSuiteEnd (suite) {
    if (config.usingAllure) {
      ...
      if (capabilities.deviceType) {
        allureReporter__default["default"].addArgument('deviceType', capabilities.deviceType);
      }
      if (capabilities.browserVersion) {
        allureReporter__default["default"].addArgument('browserVersion', capabilities.browserVersion);
      }
    }
    ...
    if (suite.type === 'scenario') {
      ...
      if (hasFailedTests || (allTestsPassed && config.saveAllVideos)) {
        ...
        browser.saveScreenshot(filePath)
          ...
          .catch((error) => {
            fs__default["default"].writeFile(filePath, notAvailableImage, 'base64');
...
class Video extends WdioReporter__default["default"] {
  /**
   * Set reporter options
   */
  constructor (options) {
    ...
    if(options.outputDir) {
      ...
      config.outputDir = options.logFile ? path__default["default"].dirname(options.logFile) : config.outputDir;
    }
  ...
  async onRunnerStart (runner) {
    ...
    if (allureConfig && allureConfig[1] && allureConfig[1].outputDir) {
      config.allureOutputDir = path__default["default"].resolve(allureConfig[1].outputDir);
    }
  ...
  onBeforeCommand () {
    if (config.usingAllure) {
      ...
      if (!this.allureVideos.includes(videoPath)) {
        ...
        allureReporter__default["default"].addAttachment('Execution video', videoPath, helpers.getVideoFileContentType());
      }
    }
  }
  ...
  onTestStart (test) {
    ...
    if (!fs__default["default"].existsSync(this.recordingPath)) {
      ...
      fs__default["default"].mkdirsSync(this.recordingPath);
    }
  ...
  onExit () {
    ...
    fs__default["default"]
      ...
      .map(filename => path__default["default"].resolve(config.allureOutputDir, filename))
      .filter(allureFile => fs__default["default"].statSync(allureFile).size < 1024)
      .filter(allureFile => this.videos.includes(fs__default["default"].readFileSync(allureFile).toString())) // Dont parse other browsers videos since they may not be ready
      .forEach((filePath) => {
        const videoFilePath = fs__default["default"].readFileSync(filePath).toString(); // The contents of the placeholder file is the video path
        if (fs__default["default"].existsSync(videoFilePath)) {
          fs__default["default"].copySync(videoFilePath, filePath);
        }
      });
  }
  ...
  addFrame () {
    ...
    const filePath = path__default["default"].resolve(this.recordingPath, frame.toString().padStart(config.screenshotPaddingWidth, '0') + '.png');
